	...
	for _, match := range matches {
		...
		for _, idmatch := range idmatches {
			...
			s1 := detectors.Result{
				...
				Raw:          []byte(resMatch),
				Description:  "Siteleaf is a content management system (CMS) designed for managing websites. Siteleaf API keys can be used to access and modify site content and settings.",
			}
