...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("theoddsapi"), []byte("the-odds-api")}
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("https://api.the-odds-api.com/v4/sports/?apiKey=%s", resMatch), nil)
			if err != nil {
				continue
			}
