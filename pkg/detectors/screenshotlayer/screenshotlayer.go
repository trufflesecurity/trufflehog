...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("screenshotlayer")}
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("https://api.screenshotlayer.com/api/capture?access_key=%s&url=https://google.com", string(resMatch)), nil)
			if err != nil {
				continue
			}
			...
			if err == nil {
				...
				if err == nil {
					validResponse := bytes.Contains(bodyBytes, []byte(`PNG`))
