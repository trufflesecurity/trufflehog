...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", "https://api.webscrapingapi.com/v1?api_key="+string(resMatch), nil)
			if err != nil {
				continue
			}
			...
			if err == nil {
				...
				if bytes.Contains(body, []byte("key `url` required.")) {
					s1.Verified = true
				} else {
