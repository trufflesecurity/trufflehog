	...
	for _, match := range matches {
		...
		for _, domainmatch := range domainMatches {
			...
			s1 := detectors.Result{
				...
				Raw:          resMatch,
				RawV2:        append(resMatch, resDomainMatch...),
			}

			if verify {
				req, err := http.NewRequestWithContext(ctx, "GET", string(resDomainMatch)+"/api/2.0/clusters/list", nil)
				if err != nil {
					continue
				}
