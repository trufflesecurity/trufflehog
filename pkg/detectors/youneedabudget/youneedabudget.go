...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			isVerified, verificationErr := verifyYouNeedABudget(ctx, client, resMatch)
...
func verifyYouNeedABudget(ctx context.Context, client *http.Client, resMatch string) (bool, error) {
