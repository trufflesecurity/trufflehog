...
func TestRootly_FromChunk(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx:    ctx,
				data:   []byte(fmt.Sprintf("You can find a rootly apikey %s within", secret)),
				verify: true,
			},
			want: []detectors.Result{
				{
		...
		},
		{
			...
			args: args{
				ctx:    ctx,
				data:   []byte(fmt.Sprintf("You can find a rootly apikey secret %s within but not valid", inactiveSecret)), // the secret would satisfy the regex but not pass validation
				verify: true,
			},
			want: []detectors.Result{
				{
