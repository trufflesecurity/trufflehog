...
func TestNewRelicUserApiKey_Integration(t *testing.T) {
	ctx := context.Background()
	s := Scanner{}
	ahoCorasickCore := ahocorasick.NewAhoCorasickCore([]detectors.Detector{s, &saucelabs.Scanner{}})
	...
	tests := []struct {
	...
	}{
		{
			...
			input:        "NEWRELIC_API_KEY=" + os.Getenv("NEWRELIC_USER_API_KEY"),
			want:         []string{os.Getenv("NEWRELIC_USER_API_KEY")},
			wantVerified: true,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if os.Getenv("NEWRELIC_USER_API_KEY") == "" {
				t.Skip("Skipping integration test: NEWRELIC_USER_API_KEY not set")
			}
