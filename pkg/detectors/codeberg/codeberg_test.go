...
func TestCodeberg_Pattern(t *testing.T) {
	d := Scanner{}
	d.SetCloudEndpoint("https://codeberg.org")
	...
	tests := []struct {
	...
	}{
		{
			...
			input: `CODEBERG_TOKEN=4ad5f3b1c9e0d27a6f8b1e2c3d4a5b6c7d8e9f01`,
			want:  []string{"4ad5f3b1c9e0d27a6f8b1e2c3d4a5b6c7d8e9f01https://codeberg.org"},
		},
		{
			name:  "valid pattern - quoted",
			input: `codeberg_api_token: "0123456789abcdef0123456789abcdef01234567"`,
			want:  []string{"0123456789abcdef0123456789abcdef01234567https://codeberg.org"},
		},
		{
		...
		},
		{
			...
			input: `codeberg token = 4AD5F3B1C9E0D27A6F8B1E2C3D4A5B6C7D8E9F01`,
			want:  nil,
		},
		{
			name:  "invalid pattern - wrong length",
			input: `codeberg token = 4ad5f3b1c9e0d27a6f8b1e2c3d4a5b6c`,
			want:  nil,
		},
	}
...
// TestCodeberg_Verification exercises the verification status codes using mock
// HTTP clients, so it requires no live credentials.
func TestCodeberg_Verification(t *testing.T) {
	const token = "9f3a7c1e0b6d48a25e1f7c93d04b8a6f2c5e9d10"
	input := []byte("codeberg token = " + token)

	newScanner := func(client *http.Client) Scanner {
		s := Scanner{client: client}
		s.SetCloudEndpoint("https://codeberg.org")
...
// TestCodeberg_Verification_Timeout covers the indeterminate failure caused by a
// network timeout.
func TestCodeberg_Verification_Timeout(t *testing.T) {
	...
	results, err := s.FromData(context.Background(), true, []byte("codeberg token = 9f3a7c1e0b6d48a25e1f7c93d04b8a6f2c5e9d10"))
	if err != nil {
		t.Fatalf("FromData() error = %v", err)
	}
	if len(results) != 1 {
