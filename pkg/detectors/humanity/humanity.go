...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, _ := http.NewRequestWithContext(ctx, "GET", "https://www.humanity.com/api/v2/me?access_token="+resMatch, nil)
			...
			if err == nil {
				...
				if !strings.Contains(body, "Invalid token key - Please re-authenticate") {
					s1.Verified = true
				} else {
