...
func TestTwitterConsumerKey_FromChunk(t *testing.T) {
	...
	secret := testSecrets.MustGetField("TWITTER_CONSUMER_SECRETS")

	inactiveKey := testSecrets.MustGetField("TWITTER_INACTIVE_CONSUMER_KEY")
	inactiveSecret := testSecrets.MustGetField("TWITTER_INACTIVE_CONSUMER_SECRETS")
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				ctx:    context.Background(),
				data:   []byte(fmt.Sprintf("You can find a twitter key %s and secret %s within", inactiveKey, inactiveSecret)), // the secret would satisfy the regex but not pass validation
				verify: true,
			},
			want: []detectors.Result{
				{
