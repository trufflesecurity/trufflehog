...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("technicalanalysisapi")}
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			...
			req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("https://technical-analysis-api.com/api/v1/analysis/BTC?apiKey=%s", string(resMatch)), nil)
			if err != nil {
				continue
			}
