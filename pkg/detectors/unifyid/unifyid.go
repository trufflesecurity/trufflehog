...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			payload := bytes.NewReader([]byte(`{"token": "sample"}`))
			...
			if err == nil {
				...
				if (res.StatusCode >= 200 && res.StatusCode < 300) || (res.StatusCode == 400 && bytes.Contains(bodyBytes, []byte("invalid token"))) {
					s1.Verified = true
				} else {
