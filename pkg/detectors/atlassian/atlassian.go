...
var (
	...
	keyPat = regexp.MustCompile(detectors.PrefixRegex([]string{"atlassian","bearer"}) + `\b([A-Za-z0-9+/=_-]+=[A-Za-z0-9]{8})`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			...
			req, err := http.NewRequestWithContext(ctx, "GET", "https://api.atlassian.com/admin/v1/orgs", nil)
			if err != nil {
				continue
			}
