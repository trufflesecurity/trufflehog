...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			reqDataQuery := bytes.NewReader([]byte(`{ "query": "{ viewer { id name } }" }`))
			...
			if err == nil {
				...
				if res.StatusCode >= 200 && res.StatusCode < 300 && bytes.Contains(bodyBytes, []byte(`"data":`)) {
					s1.Verified = true
				} else {
