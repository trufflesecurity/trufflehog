...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("screenshotapi")}
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", "https://shot.screenshotapi.net/screenshot?token="+string(resMatch)+"&url=https://google.com&width=1920&height=1080&output=image", nil)
			if err != nil {
				continue
			}
