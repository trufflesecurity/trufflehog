...
func TestTeleriklicensekey_Pattern(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Valid JWT - Valid Telerik",
			input: `
				[INFO] Checking license key...
				[DEBUG] Using eyJhbGciOiJSUzI1NiIsInR5cCI6IlRlbGVyaWsgTGljZW5zZSBLZXkifQ.eyJ0eXBlIjoidGVs....
				[INFO] Result: Valid JWT with Telerik License Key
			`,
		...
		{
			name: "Valid JWT - Invalid Telerik",
			input: `
				...
				[ERROR] Response received: Valid JWT, but NOT a Telerik License Key
			`,
