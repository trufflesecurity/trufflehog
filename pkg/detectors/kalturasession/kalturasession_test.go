...
func TestKalturaSession_FromChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("KalturaSession.FromData() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			for i := range got {
				if len(got[i].Raw) == 0 {
			...
			if diff := pretty.Compare(got, tt.want); diff != "" {
				t.Errorf("KalturaSession.FromData() %s diff: (-got +want)\n%s", tt.name, diff)
			}
		})
	}
}
