...
func TestNeon_FromChunk(t *testing.T) {
	...
	secret := testSecrets.MustGetField("NEON")
	inactiveSecret := testSecrets.MustGetField("NEON_INACTIVE")
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				data:   []byte(fmt.Sprintf("You can find a neon secret %s within", secret)),
				verify: true,
			},
			want: []detectors.Result{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Neon.FromData() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			for i := range got {
				if len(got[i].Raw) == 0 {
