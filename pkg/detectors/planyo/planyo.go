...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, _ := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("https://www.planyo.com/rest/?method=get_site_info&api_key=%s", resMatch), nil)
			...
			if err == nil {
				...
				if res.StatusCode >= 200 && res.StatusCode < 300 && !strings.Contains(body, "Invalid method or API key") {
					s1.Verified = true
				} else {
