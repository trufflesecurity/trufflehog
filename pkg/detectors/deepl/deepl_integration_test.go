...
func TestDeepl_FromChunk(t *testing.T) {
	...
	secret := testSecrets.MustGetField("DEEPL")
	inactiveSecret := testSecrets.MustGetField("DEEPL_INACTIVE")
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				data:   []byte(fmt.Sprintf("You can find a deepl secret %s within", secret)),
				verify: true,
			},
			want: []detectors.Result{
				{
		...
		},
		{
			...
			args: args{
				ctx:    context.Background(),
				data:   []byte(fmt.Sprintf("You can find a deepl secret %s within but not valid", inactiveSecret)),
				verify: true,
			},
			want: []detectors.Result{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("DeepL.FromData() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			for i := range got {
				if len(got[i].Raw) == 0 {
