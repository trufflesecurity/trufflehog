...
func TestMake_FromChunk(t *testing.T) {
	...
	endpoint := testSecrets.MustGetField("MAKE_API_ENDPOINT")
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				data:   []byte(fmt.Sprintf("You can find a make secret %s and endpoint %s", secret, endpoint)),
				verify: true,
			},
			want: []detectors.Result{
				{
		...
		},
		{
			...
			args: args{
				ctx:    context.Background(),
				data:   []byte(fmt.Sprintf("You can find a make secret %s and endpoint %s but not valid", inactiveSecret, endpoint)), // the secret would satisfy the regex but not pass validation
				verify: true,
			},
			want: []detectors.Result{
				{
