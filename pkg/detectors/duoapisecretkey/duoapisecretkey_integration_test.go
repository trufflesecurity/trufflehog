...
func TestDuoapisecretkey_FromChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Duoapisecretkey.FromData() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			for i := range got {
				if len(got[i].Raw) == 0 {
			...
			if diff := cmp.Diff(got, tt.want, ignoreOpts); diff != "" {
				t.Errorf("Duoapisecretkey.FromData() %s diff: (-got +want)\n%s", tt.name, diff)
			}
		})
	}
}
