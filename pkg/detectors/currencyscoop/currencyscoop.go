...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("currencyscoop")}
}
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", fmt.Sprintf("https://api.currencyscoop.com/v1/latest?api_key=%s", string(resMatch)), nil)
			if err != nil {
				continue
			}
