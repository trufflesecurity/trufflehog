...
func (s Scanner) Keywords() [][]byte {
	return [][]byte{[]byte("datadog")}
}

func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	matches := keyPat.FindAllSubmatch(data, -1)
	apiMatches := apiPat.FindAllSubmatch(data, -1)
	...
	for _, match := range matches {
		...
		for _, apimatch := range apiMatches {
			...
			resApiMatch := bytes.TrimSpace(apimatch[1])

			s1 := detectors.Result{
				DetectorType: detectorspb.DetectorType_DatadogToken,
				Raw:          resMatch,
				RawV2:        append(resMatch, resApiMatch...),
			}
			...
			if verify {
				...
				req.Header.Add("DD-API-KEY", string(resApiMatch))
				req.Header.Add("DD-APPLICATION-KEY", string(resMatch))
