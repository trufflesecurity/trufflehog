...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			req, err := http.NewRequestWithContext(ctx, "GET", "https://"+string(resMatch)+"@sonarcloud.io/api/authentication/validate", nil)
			if err != nil {
				continue
			}
			...
			if err == nil {
				...
				validResponse := bytes.Contains(bodyBytes, []byte(`"valid":true`))
