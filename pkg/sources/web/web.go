...
func (s *Source) Init(ctx context.Context, name string, jobId sources.JobID, sourceId sources.SourceID,
	...
	if s.conn.GetDepth() < 0 {
		return fmt.Errorf("invalid depth %d: must be 0 (unlimited) or positive", s.conn.GetDepth())
	}
	if s.conn.GetDelay() < 0 {
		return fmt.Errorf("invalid delay %d: must be zero or positive", s.conn.GetDelay())
	}
...
func (s *Source) Chunks(ctx context.Context, chunksChan chan *sources.Chunk, _ ...sources.ChunkingTarget) error {
	...
	for _, u := range s.conn.GetUrls() {
		...
		eg.Go(func() error {
			// Each URL gets its own timeout budget so the --timeout flag applies
			// per URL, not to the combined crawl of every seed URL.
...
func (s *Source) crawlURL(ctx context.Context, seedURL string, chunksChan chan *sources.Chunk) error {
	...
	if s.conn.GetCrawl() {
		visitDiscoveredURL := func(e *colly.HTMLElement, rawURL, kind string) {
			...
			if err := e.Request.Visit(link); err != nil {
				if _, ok := err.(*colly.AlreadyVisitedError); !ok {
					ctx.Logger().V(5).Info("Skipping "+kind, "url", link, "reason", err)
				}
			}
		}

		collector.OnHTML("a[href]", func(e *colly.HTMLElement) {
			visitDiscoveredURL(e, e.Attr("href"), "link")
		})
		...
		collector.OnHTML("script[src]", func(e *colly.HTMLElement) {
			visitDiscoveredURL(e, e.Attr("src"), "script")
		})
	}

	// Buffered so the crawl goroutine can always deliver its result and exit,
	// even if we stop waiting on it below (e.g. collector.Wait() never
	// returns because colly does not guarantee prompt reaction to context
	// cancellation). An unbuffered channel would otherwise leak this
	...
	case err := <-done:
		...
		return nil
	case <-ctx.Done():
		// Don't block on <-done here: collector.Wait() is not guaranteed to
		// return promptly on cancellation. Return immediately and let the
		// crawl goroutine finish (or leak) on its own; the buffered channel
		// above ensures it won't itself deadlock.
