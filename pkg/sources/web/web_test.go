...
func TestInit_NegativeDepth(t *testing.T) {
	conn := &anypb.Any{}
	require.NoError(t, conn.MarshalFrom(&sourcespb.Web{
		Urls:  []string{"http://example.com"},
		Crawl: true,
...
func TestInit_NegativeDelay(t *testing.T) {
	conn := &anypb.Any{}
	require.NoError(t, conn.MarshalFrom(&sourcespb.Web{
		Urls:  []string{"http://example.com"},
		Delay: -1,
...
func TestChunks_PartialFailure(t *testing.T) {
	...
	goodSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `<html><head><title>Good</title></head><body>Good data</body></html>`)
	}))
	defer goodSrv.Close()
...
func TestChunks_HappyPath(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `<!DOCTYPE html><html><head><title>Test Page</title></head><body>Hello, world!</body></html>`)
	}))
	defer srv.Close()
...
func TestChunks_TimestampIsRFC3339(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprint(w, `<html><head><title>T</title></head><body>body</body></html>`)
	}))
	defer srv.Close()
...
func TestChunks_NoCrawl(t *testing.T) {
	...
	mux.HandleFunc("/page2", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><head><title>Page 2</title></head><body>Secret</body></html>`)
	})
...
func TestChunks_CrawlDepth1(t *testing.T) {
	...
	mux.HandleFunc("/child", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><head><title>Child</title></head><body>
			<a href="/grandchild">grandchild</a>
		</body></html>`)
	})
	mux.HandleFunc("/grandchild", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><head><title>Grandchild</title></head><body>deep</body></html>`)
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
...
func TestChunks_CrossDomainLinksIgnored(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprintf(w, `<html><head><title>Seed</title></head><body>
...
func TestChunks_PlainTextPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, "API_KEY=supersecret")
	}))
	defer srv.Close()
...
func TestChunks_JSONPage(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"token":"ghp_supersecret"}`)
	}))
	defer srv.Close()
...
func TestChunks_DuplicateLinksVisitedOnce(t *testing.T) {
	...
	mux.HandleFunc("/dup", func(w http.ResponseWriter, r *http.Request) {
		hitCount.Add(1)
		_, _ = fmt.Fprint(w, `<html><head><title>Dup</title></head><body>dup</body></html>`)
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
	...
	assert.Equal(t, int32(1), hitCount.Load(), "duplicate links should only be fetched once")
}
...
func TestChunks_RobotsTxtRespected(t *testing.T) {
	...
	var secretVisited atomic.Bool
	mux.HandleFunc("/secret", func(w http.ResponseWriter, r *http.Request) {
		secretVisited.Store(true)
		_, _ = fmt.Fprint(w, `<html><head><title>Secret</title></head><body>private</body></html>`)
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()
	...
	assert.False(t, secretVisited.Load(), "/secret must not be crawled when disallowed by robots.txt")
}
...
func TestChunks_IgnoreRobotsTxt(t *testing.T) {
	...
	assert.True(t, secretVisited.Load(), "/secret should be crawled when IgnoreRobots=true")
}
