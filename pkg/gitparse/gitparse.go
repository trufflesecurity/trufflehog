...
func (c *Parser) RepoPath(
	...
	// different goroutine after the command funishes, but we're not
	...
	go func() {
		...
		for group := range commitGroups {
			...
			showCmd := exec.CommandContext(ctx,
				"git",
				slices.Concat(args.global, []string{"show"}, args.show, group)...,
			)
			showCmd.Env = args.env

			diffGroup, err := c.executeCommand(ctx, showCmd, false, c.waitDelay)
			if err != nil {
...
func (c *Parser) prepGitArgs(source string, abbrevCommit int, head string, abbreviatedLog bool, excludedGlobs []string, isBare bool) gitArgs {
	...
	if len(excludedGlobs) != 0 {
		...
		for _, glob := range excludedGlobs {
			...
			paths = append(paths, ":(exclude)"+glob)
		}

		args.log = append(args.log, paths...)
		args.show = append(args.show, paths...)
	}
