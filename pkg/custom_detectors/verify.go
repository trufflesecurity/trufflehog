...
func VerifyCredential(
	...
	for _, ep := range endpoints {
		...
		// Read and close the response body. Capture it for the outcome
		// in case this endpoint produces the definitive answer.
		...
		if len(successRanges) == 0 && len(rotatedRanges) == 0 {
			...
			if resp.StatusCode == http.StatusOK {
				...
				outcome.RespBody = truncateBody(respBody)
				break
			}
			outcome.Definitive = true
			continue
...
func truncateBody(b []byte) string {
	s := string(b)
	if len(s) > maxRespBodyLen {
		return s[:maxRespBodyLen]
	}
	return s
}
