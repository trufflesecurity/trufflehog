...
2. **Define the Custom Detector**:
   ...
   **Other allowed parameters:**
   ...
   - **`exclude_regexes_capture`**: This parameter enables you to define regex patterns to exclude certain captured tokens from being reported as secrets. The pattern is tested against only the captured token, and not the entire matched string.
   - **`exclude_regexes_match`**: This parameter enables you to define regex patterns to exclude certain matched strings from being reported as secrets. The pattern is tested against entire matched string, not just the captured token.
