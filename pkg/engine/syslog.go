...
func (e *Engine) ScanSyslog(ctx context.Context, c sources.SyslogConfig) error {
	...
	handle, err := e.sourceManager.Enroll(ctx, "trufflehog - syslog", (&syslog.Source{}).Type(),
		func(ctx context.Context, jobID, sourceID int64) (sources.Source, error) {
			source := &syslog.Source{}
			if err := source.Init(ctx, "trufflehog - syslog", jobID, sourceID, false, &conn, c.Concurrency); err != nil {
				return nil, err
			}
			return source, nil
		})
