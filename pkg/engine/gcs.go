...
func (e *Engine) ScanGCS(ctx context.Context, c sources.GCSConfig) error {
	...
	handle, err := e.sourceManager.Enroll(ctx, "trufflehog - GCS", (&gcs.Source{}).Type(),
		func(ctx context.Context, jobID, sourceID int64) (sources.Source, error) {
			source := &gcs.Source{}
			if err := source.Init(ctx, "trufflehog - GCS", jobID, sourceID, true, &conn, int(c.Concurrency)); err != nil {
				return nil, err
			}
			return source, nil
		})
