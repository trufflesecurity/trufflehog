...
func (e *Engine) ScanGit(ctx context.Context, c sources.GitConfig) error {
	...
	handle, err := e.sourceManager.Enroll(ctx, "trufflehog - git", (&git.Source{}).Type(),
		func(ctx context.Context, jobID, sourceID int64) (sources.Source, error) {
			...
			// Don't try to clean up the provided directory. That's
			// handled by the caller of ScanGit.
