...
def get_org_repos(orgname, page):
    response = requests.get(
        url='https://api.github.com/users/' + orgname + '/repos?page={}'.format(page))
    json = response.json()
    if not json:
        return None
    for item in json:
        if item['private'] == False:
            ...
            output = truffleHog.find_strings(
                item["html_url"], do_regex=True, do_entropy=False, max_depth=100000)
            truffleHog.print_results(output['foundIssues'])
